...
def simulate_dynamics(model: Model, simulation_setup_and_state: SimulationSetupAndState, print_at_each_integration_step: Union[Callable, None] = None, print_at_each_simulation_step: Union[Callable, None] = None, print_at_end_of_simulation: Union[Callable, None] = None) -> None: # Main function to simulate the dynamics of the model until the end condition is met
	...
	while True:
		...
		p0 = simulation_setup_and_state.rng_Gillespie.random() # Generate a random number for event time calculation
		...
		p1 = simulation_setup_and_state.rng_Gillespie.random() # Generate a random number for event selection
		...
		if event_index < events_indices[0]: # RNAP recruitment event
			...
			chosen_segment_index = select_event_based_on_propensities(per_segment_on_rates, simulation_setup_and_state.rng_everything_else.random())
			binding_position = model.genomic_setup.clamp_left + sum(segments_lengths[chosen_segment_index + 1:]) + (segments_lengths[chosen_segment_index]*simulation_setup_and_state.rng_everything_else.random())
			...
			chosen_bound_protein_index = select_event_based_on_propensities(binding_proteins_off_rates, simulation_setup_and_state.rng_everything_else.random())
